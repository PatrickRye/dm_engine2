...
MASTER_TOOLS_LIST = [
    ...
    interact_with_object, manage_map_trap, manage_skill_challenge, generate_random_loot, ingest_battlemap_json, manage_map_terrain
]
...
async def planner_node(state: DMState, config: RunnableConfig):
    sys_msg = SystemMessage(content="""
        ...
        2. SPELLS & CLASS FEATURES: Always use `use_ability_or_spell`. Never roll manually. If the player specifies an Area of Effect (AoE) with coordinates, pass `target_x`, `target_y`, `aoe_shape`, and `aoe_size` to let the engine automatically resolve line-of-sight, calculate exact hits, and damage destructible walls.
        ...
        12. ENVIRONMENT: You can cast spells or cause effects that alter the environment. Use `manage_map_terrain` to add water, ice, webs, or spikes. The engine will automatically ignite flammable webs with fire, melt ice, or electrify water with lightning!
        
        MOVEMENT PARADIGMS:
        - TRAVEL / TOWN (Out of Combat): Use `move_entity(movement_type='travel')` to loosely update locations across large distances. Distance limits and map walls are ignored.
        - DUNGEON CRAWL (Out of Combat): Use `move_entity(movement_type='walk')`. Movement budgets are ignored, but doors, traps, and walls interact correctly.
...
async def qa_node(state: DMState, config: RunnableConfig):
    ...
    qa_prompt = (
        ...
        "11. PARADIGMS: If players are exploring a dungeon out of combat, did the DM force them to stay within 30ft movement limits? (If yes, REJECT. Movement budgets only apply in combat).\n"
        + tone_check +
    ...
    for i in range(1, len(detailed_path)):
        ...
        if collision:
            ...
            alternative_path = [
                ...
                ((collision.start[0] - 5) * pixels_per_foot, (collision.start[1] - 5) * pixels_per_foot),
        ...
        if hasattr(entity, 'movement_remaining') and is_in_combat:
            ...
            if truncated_cost > entity.movement_remaining:
                ...
                invalid_reason = f"Movement cost ({movement_cost:.1f} ft) exceeds remaining speed ({entity.movement_remaining} ft)."
                break
        ...
        if trap_hit:
            ...
            if not request.force_execute: break
