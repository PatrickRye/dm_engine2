...
def get_entity(uid: uuid.UUID, vault_path: str = None) -> Optional[Any]:
    ...
    for vp, vault in _ACTIVE_ENTITIES.items():
        if uid in vault: return vault[uid]
    return None
...
def clear_registry(vault_path: str = None) -> None:
    ...
    if vault_path:
        if vault_path in _ACTIVE_ENTITIES: _ACTIVE_ENTITIES[vault_path].clear()
        if vault_path in _NAME_INDEX: _NAME_INDEX[vault_path].clear()
    else:
