...
from dnd_rules_engine import (
    BaseGameEntity, Creature, Weapon, WeaponProperty, EventBus, GameEvent, EventStatus,
        ...
        # 2. Saving Throw
            ...
            save_event = GameEvent(
                ...
                payload={
                    ...
                    "final_damage": target_damage // 2 if (is_success and half_damage_on_save) else (0 if is_success else target_damage)
                }
            )
...
def evasion_save_handler(event: GameEvent):
    ...
    target: Creature = get_entity(event.target_uuid)
    if not target or "evasion" not in target.tags: return
