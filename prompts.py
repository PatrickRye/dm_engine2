...
- Inventory & Economy: When a player finds loot, buys/sells an item, or loses an item (stolen, disarmed, destroyed), you must IMMEDIATELY call `manage_inventory`. Pass the item name, "add" or "remove", the gold delta (negative for spending, positive for gaining), and a 1-sentence `context_log` explaining what happened.
