...
from dnd_rules_engine import BaseGameEntity, Creature, EventBus
from vault_io import initialize_engine_from_vault
from tools import (
    start_combat, update_combat_state, end_combat,
    move_entity, execute_melee_attack, use_ability_or_spell,
...
def mock_5_round_vault(mock_obsidian_vault):
    ...
    fireball = {
        "name": "Fireball", "category": "spell", "action_type": "Action", "description": "Boom.",
        "mechanics": { "damage_dice": "8d6", "damage_type": "fire", "save_required": "dexterity", "half_damage_on_save": True }
    }
    with open(os.path.join(comp_dir, "fireball.json"), "w", encoding="utf-8") as f: json.dump(fireball, f)
    
    registry = {"fireball": os.path.join(comp_dir, "fireball.json")}
    with open(os.path.join(mock_obsidian_vault, "Compendium", "registry.json"), "w", encoding="utf-8") as f: json.dump(registry, f)
...
async def test_5_round_combat_execution(mock_5_round_vault):
    ...
    with patch('random.randint', side_effect=lambda a, b: b // 2 + 1):
        ...
        # --- ROUND 1 ---
        res = await use_ability_or_spell.ainvoke({"caster_name": "Wizard", "ability_name": "Fireball", "target_names": ["Troll", "Orc1", "Orc2"]}, config=config)
        assert "Troll" in res and "Orc1" in res
        
        await move_entity.ainvoke({"entity_name": "Fighter", "target_x": 25, "target_y": 0, "movement_type": "walk"}, config=config)
        ...
        # --- ROUND 3 ---
        await move_entity.ainvoke({"entity_name": "Monk", "target_x": 25, "target_y": 5, "movement_type": "walk"}, config=config)
        await execute_melee_attack.ainvoke({"attacker_name": "Monk", "target_name": "Goblin"}, config=config)
        await update_combat_state.ainvoke({"next_turn": True}, config=config)
        
        # --- ROUND 4 ---
        await move_entity.ainvoke({"entity_name": "Thief", "target_x": 25, "target_y": -5, "movement_type": "walk"}, config=config)
