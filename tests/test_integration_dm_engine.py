...
from dnd_rules_engine import Creature, MeleeWeapon
...
async def test_tool_to_engine_sync(mock_entities):
    ...
    result = await equip_item.ainvoke(
        {"character_name": char_name, "item_name": "Silver Longsword", "item_slot": "main_hand", "new_ac_value": 17},
        config=config,
    )
...
async def test_tool_level_up_character(mock_entities):
    ...
    result = await level_up_character.ainvoke(
        {"character_name": char_name, "class_name": "Fighter", "hp_increase": 8}, config=config
    )
...
async def test_tool_ability_check_engine_coupling(mock_entities):
    ...
    tharion.dexterity_mod.add_modifier(
        NumericalModifier(priority=ModifierPriority.ADDITIVE, value=5, source_name="Cat's Grace")
    )

    # Tharion's base DEX mod in YAML is 1. With buff, it should be 6.
    with patch("random.randint", return_value=10):
        res = await perform_ability_check_or_save.ainvoke(
            {"character_name": char_name, "skill_or_stat_name": "stealth"}, config=config
        )
...
async def test_tool_modify_health_respects_resistances(mock_entities):
    ...
    res = await modify_health.ainvoke(
        {"target_name": char_name, "hp_change": -20, "reason": "Lava Pit", "damage_type": "fire"}, config=config
    )
...
async def test_tool_opportunity_attack_integration(mock_entities):
    ...
    request_data = {"entity_name": target_name, "waypoints": [[5.0, 0.0], [15.0, 0.0]], "vault_path": vault_path}
    ...
    with patch("random.randint", side_effect=[18, 18, 4]):  # Attack roll 18, Damage roll 4
        oa_result = await execute_melee_attack.ainvoke(
            {"attacker_name": char_name, "target_name": target_name, "is_opportunity_attack": True}, config=config
        )
...
async def test_tool_manage_skill_challenge(mock_entities):
    ...
    res1 = await manage_skill_challenge.ainvoke(
        {"action": "start", "name": "Escape the Cave", "max_successes": 3, "max_failures": 3}, config=config
    )
    assert "started" in res1

    res2 = await manage_skill_challenge.ainvoke(
        {"action": "update", "successes_delta": 1, "note": "Jumped a chasm"}, config=config
    )
