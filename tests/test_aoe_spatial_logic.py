...
def test_fireball_destructible_wall():
    ...
    for i in range(10):
        ...
        w = Wall(
            ...
            start=(start_x, 0), end=(end_x, 0),
            is_solid=True, hp=10, max_hp=10, vulnerabilities=["fire"]
        )
...
def test_fireball_evasion_mechanics():
    ...
    with patch('event_handlers.roll_dice', return_value=30), patch('random.randint', return_value=5):
        event = GameEvent(event_type="SpellCast", source_uuid=caster.entity_uuid, payload={"ability_name": "Fireball", "mechanics": mechanics, "target_uuids": hits, "target_wall_ids": []})
        EventBus.dispatch(event)
    ...
    with patch('event_handlers.roll_dice', return_value=30), patch('random.randint', return_value=10):
        EventBus.dispatch(GameEvent(event_type="SpellCast", source_uuid=caster.entity_uuid, payload={"ability_name": "Fireball", "mechanics": mechanics, "target_uuids": hits, "target_wall_ids": []}))
...
def test_aoe_z_axis_cone_upward():
    ...
    assert c_flying_above.entity_uuid not in hits, "Flying enemy directly above the caster should be outside the 53 deg cone angle."
...
def test_aoe_lightning_bolt_destructible_door():
    ...
    with patch('event_handlers.roll_dice', return_value=30), patch('random.randint', return_value=5): 
        event = GameEvent(event_type="SpellCast", source_uuid=caster.entity_uuid, payload={"ability_name": "Lightning Bolt", "mechanics": mechanics, "target_uuids": hits_door, "target_wall_ids": hit_walls_door})
        EventBus.dispatch(event)
