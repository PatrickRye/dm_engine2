...
async def test_execute_melee_attack_reach_enforcement():
    ...
    res = await execute_melee_attack.ainvoke(
        {"attacker_name": "Attacker", "target_name": "Target", "is_opportunity_attack": True}, config=config
    )
    assert "out of range" in res
...
def test_large_creature_reach(client):
    ...
    # Large enemy (size 10.0), no reach weapons. Base reach = 5. Effective center-to-center distance boundary = 5 + (10-5)/2 = 7.5  # noqa: E501
    enemy = Creature(
        name="Enemy",
        x=12.5,
        y=0.0,
