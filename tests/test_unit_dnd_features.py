...
from spatial_engine import spatial_service, Wall, TerrainZone, LightSource
...
def test_multi_turn_expiration_and_premature_checks():
    ...
    mod = NumericalModifier(
        priority=ModifierPriority.ADDITIVE, value=2, source_name="Bless", duration_seconds=18, applied_initiative=15
    )
...
def test_reaction_limit_enforcement():
    ...
    with patch("random.randint", return_value=10):
        ...
        EventBus.dispatch(
            GameEvent(event_type="MeleeAttack", source_uuid=attacker.entity_uuid, target_uuid=wizard.entity_uuid)
        )
...
def test_concentration_auto_drop_on_zero_hp():
    ...
    with patch.object(EventBus, "dispatch", wraps=EventBus.dispatch) as mock_dispatch:
        ...
        dmg_event = GameEvent(
            ...
            target_uuid=pc.entity_uuid,
            payload={"hit": True, "damage": 10, "damage_type": "slashing"},
        )
        ...
        drop_events = [
            ...
            if call_args.args[0].event_type == "DropConcentration"
        ]
...
def test_opportunity_attack_bypass():
    ...
    walk_event = GameEvent(
        event_type="Movement",
        source_uuid=pc.entity_uuid,
        payload={"target_x": 15, "target_y": 0, "target_z": 0, "movement_type": "walk"},
    )
    EventBus.dispatch(walk_event)
    ...
    tp_event = GameEvent(
        event_type="Movement",
        source_uuid=pc.entity_uuid,
        payload={"target_x": 15, "target_y": 0, "target_z": 0, "movement_type": "teleport"},
    )
...
def test_evasion_and_concentration_check_prompt():
    ...
    rogue = Creature(
        ...
        concentrating_on="Invisibility",
    )

    spatial_service.sync_entity(caster)
    spatial_service.sync_entity(rogue)

    mechanics = {"save_required": "dexterity", "damage_dice": "8d6", "damage_type": "fire", "half_damage_on_save": True}

    with patch(
        "random.randint", side_effect=[1, 1, 1, 1, 1, 1, 1, 1, 2, 2]
    ):  # 8 base dmg, save roll 2 (roll1), 2 (roll2) + 5 = 7 (Fail)
        with patch("builtins.print") as mock_print:
            EventBus.dispatch(
                GameEvent(
                    event_type="SpellCast",
                    source_uuid=caster.entity_uuid,
                    payload={"ability_name": "Fireball", "mechanics": mechanics, "target_uuids": [rogue.entity_uuid]},
                )
            )
