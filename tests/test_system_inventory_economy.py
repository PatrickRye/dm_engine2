...
async def test_inventory_buying_and_selling(mock_inventory_entity):
    ...
    res_buy_pc = await manage_inventory.ainvoke({
        "character_name": char_name, "item_name": "Torch", "action": "add", 
        "quantity": 5, "cp_change": -5, "context_log": "Bought 5 torches from Merchant."
    }, config=config)
    
    # Merchant sells 5 torches for 5cp total
    res_buy_npc = await manage_inventory.ainvoke({
        "character_name": npc_name, "item_name": "Torch", "action": "remove", 
        "quantity": 5, "cp_change": 5, "context_log": "Sold 5 torches to Rogue."
    }, config=config)
...
async def test_inventory_currency_exchange_downconversion(mock_inventory_entity):
    ...
    res = await manage_inventory.ainvoke({
        "character_name": char_name, "item_name": "Silver Dagger", "action": "add", 
        "sp_change": -30, "context_log": "Bought a dagger."
    }, config=config)
...
async def test_inventory_insufficient_funds(mock_inventory_entity):
    ...
    res = await manage_inventory.ainvoke({
        "character_name": char_name, "item_name": "Plate Armor", "action": "add", 
        "gp_change": -50, "context_log": "Tried to buy Plate Armor."
    }, config=config)
...
async def test_inventory_consumable_usage(mock_inventory_entity):
    ...
    await manage_inventory.ainvoke({
        "character_name": char_name, "item_name": "Potion of Healing", "action": "remove", 
        "quantity": 1, "context_log": "Drank a potion."
    }, config=config)
    
    # Rogue shoots 5 Arrows (Started with 20)
    await manage_inventory.ainvoke({
        "character_name": char_name, "item_name": "Arrow", "action": "remove", 
        "quantity": 5, "context_log": "Shot 5 arrows in combat."
    }, config=config)
...
async def test_inventory_stealing_looting_and_bartering(mock_inventory_entity):
    ...
    await manage_inventory.ainvoke({"character_name": char_name, "item_name": "Shortbow", "action": "remove", "quantity": 1}, config=config)
    await manage_inventory.ainvoke({"character_name": npc_name, "item_name": "Shortbow", "action": "add", "quantity": 1}, config=config)
    
    await manage_inventory.ainvoke({"character_name": npc_name, "item_name": "Torch", "action": "remove", "quantity": 10}, config=config)
    await manage_inventory.ainvoke({"character_name": char_name, "item_name": "Torch", "action": "add", "quantity": 10}, config=config)
