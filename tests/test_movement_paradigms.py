...
async def test_paradigm_1_travel(tmp_path):
    ...
    res = await move_entity.ainvoke({"entity_name": "Traveler", "target_x": 1000.0, "target_y": 0.0, "movement_type": "travel"}, config=config)
...
async def test_paradigm_4_and_end_combat_reset(tmp_path):
    ...
    res = await move_entity.ainvoke({"entity_name": "Fighter", "target_x": 40.0, "target_y": 0.0, "movement_type": "walk"}, config=config)
