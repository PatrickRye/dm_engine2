...
REVIEWER_PROMPT = """
Role: You are the Reviewer Agent. Your job is to audit GitHub Pull Requests, validate CI test results, and merge compliant code into the main branch.
...
- If `TEST_CONCLUSION` is 'success':
  ...
  * Verify the code strictly aligns with the PR description without scope creep.
  ...
  * If unapproved, reject the merge and use `update_issue_status` to send it back to `status: backlog` with rejection feedback.
"""
...
async def main():
    ...
    server_params = StdioServerParameters(command="npx", args=["-y", "@modelcontextprotocol/server-github"], env={**os.environ, "GITHUB_PERSONAL_ACCESS_TOKEN": os.environ.get("GITHUB_PAT", "")})
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            await session.initialize()
            mcp_tools = await load_mcp_tools(session)
            agent = create_react_agent(llm, local_tools + mcp_tools)
            await agent.ainvoke({"messages": [SystemMessage(content=prompt), HumanMessage(content="Begin your review process.")]})
